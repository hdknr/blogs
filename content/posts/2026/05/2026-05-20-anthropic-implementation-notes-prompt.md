...
categories: ["AI/LLM"]
tags: ["claude-code", "anthropic", "プロンプトエンジニアリング", "意思決定ログ", "OpenSpec"]
---
...
[OpenSpec](/blogs/posts/2026/05/openspec-spec-driven-development/) は変更ごとに `proposal.md`（Why）/ `design.md`（How）/ `tasks.md`（Do）/ `specs/`（What）の 4 アーティファクトを生成する仕様駆動開発フレームワークだ。

`implementation-notes.md` は、ここに **第5のアーティファクトとして自然に組み込める**。

```text
docs/changes/2026-05-20-jwt-auth/
├── proposal.md           # Why: なぜ JWT 認証を入れるか
├── design.md             # How: アルゴリズム選定、フロー設計
├── tasks.md              # Do: 実装チェックリスト
└── implementation-notes.md  # ← 実装中の判断・逸脱をここに残す
```

この配置のメリット:

- **計画と実装ノートが同じディレクトリで対比できる** — `design.md` で「HS256 を採用」と書き、`implementation-notes.md` に「予定通り HS256」「ただし鍵長は仕様外で 256bit を選択」と残せる
- **archive 時にスナップショットとして固定される** — `docs/changes/archive/2026-05-20-jwt-auth/` に移送されると、その時点の判断がそのまま残る
- **AI に「該当 change ディレクトリだけ読ませる」運用と整合** — 並行開発中の他機能のノートが混入しない
- **PR レビュー時に「計画 → 実装 → ノート」が1ディレクトリで完結する** — レビュアは proposal → design → tasks → notes の順に読めば全文脈が得られる

### 既存 docs/ 運用でも構造だけ真似られる

OpenSpec を導入していなくても、すでに mkdocs + `docs/` 運用しているプロジェクトなら、**ディレクトリ構造だけ真似る** だけで同じ効果が得られる。具体的なリポジトリ規約の書き方は OpenSpec 記事の「[docs-structure.md + GitHub Issue で初期化する](/blogs/posts/2026/05/openspec-spec-driven-development/)」を参照のこと。

### `implementation-notes.md` のテンプレート

```markdown
# Implementation Notes — JWT 認証

## 仕様外の判断

### 鍵長: 256bit を採用
- 仕様には記載なし
- 理由: セキュリティとパフォーマンスのバランス。128bit は将来の量子耐性で懸念

### リフレッシュトークンの有効期限: 7日
- 仕様には「適切な期間」とだけ記載
- 理由: UX 上のセッション切れ頻度とセキュリティのバランス

## 計画からの逸脱

### design.md では Redis ベースのトークン無効化リストを採用予定だったが、PostgreSQL に変更
- 理由: 現プロジェクトには Redis インフラがなく、追加コストが見合わない
- トレードオフ: 無効化チェックの遅延が +5ms 程度（許容範囲）
- 将来 Redis を導入する場合は再検討

## 妥協点

### エラーハンドリングの粒度
- 仕様で詳細指定がなかったため、4xx / 5xx の区別のみ実装
- 詳細なエラーコード体系は次フェーズの課題として `tasks.md` 末尾に追記

## 将来要再検討

- HS256 → RS256 への移行（マルチサービス化の際）
- トークンローテーション戦略
```
...
- 残し方は [OpenSpec 流の変更単位ディレクトリ](/blogs/posts/2026/05/openspec-spec-driven-development/) に第 5 のアーティファクトとして置くのが構造的に最も強い
