...
categories: ["ツール/開発環境"]
tags: ["dmux", "Claude Code", "AI エージェント", "git worktree", "並列実行"]
---
...
dmux は、AI エージェントの並列実行における「ファイル競合」という実用上の課題に対して、git worktree という既存の Git 機能を活かしてシンプルに解決するツール。Claude Code や Codex を使った開発フローを並列化・効率化したい場合に有力な選択肢となる。
