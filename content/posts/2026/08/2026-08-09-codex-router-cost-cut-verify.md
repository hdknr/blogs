...
description: "「codex-router で DeepSeek を Codex のサブエージェントにすれば高額サブスク不要」という拡散投稿を検証。モデル指定は Codex 純正の設定だけででき、ルーターが足すのは一覧統合と認証分離。価格差も整理する。"
categories: ["AI/LLM"]
tags: ["Codex", "codex-router", "DeepSeek", "サブエージェント", "コスト削減", "OpenAI", "ファクトチェック"]
---
...
なお、これは **Codex の話**である。Claude Code には「スワームの1体だけ別プロバイダに向ける」フックがなく、`model` の値は Claude のティアに限定された enum になっている。同じ「安いモデルに委譲する」でも、製品によって打てる手が違う。
