...
### Node.js が要るのはエージェントのためではない

ここは混同しやすい。**Claude Code も Codex CLI も、いまは Node.js を必要としない。** どちらも公式に推奨されているのはネイティブバイナリのインストーラで、Node ランタイムに依存しない。

にもかかわらず接続時に `Node.js not found on remote host` が出るのは、**Orca 自身がリモートに置くリレーが Node アプリケーションだから**だ。リポジトリの `src/relay/` にあり、ファイル名から責務を拾うとこうなる。

| 領域 | 役割 |
| --- | --- |
| `pty` | 疑似端末の生成・入出力・再アタッチ |
| `fs` | ファイル読み書きとストリーミング |
| `git` | worktree 操作、diff、コミット |
| `agent` | エージェントプロセスの起動と hook |
| `dispatcher` | フレーム多重化、バックプレッシャ制御 |

つまり手元の Orca は UI で、**リモート側で実際にファイルを触り git を叩きエージェントを起動しているのはこの Node プロセス**になる。だから Node.js が無いと、エージェントを起動する以前の段階で接続が成立しない。

なお `package.json` は `engines.node` に `24`、`node-pty` に `^1.1.0` を指定している（確認時点のバージョンは 1.4.178-rc.2）。元記事が `nvm install --lts` を指示しているのは、これと辻褄が合う。

### ターミナルだけがネイティブモジュールに依存する

リレーの責務のうち、`node-pty` を要求するのは**ターミナルだけ**だ。`src/relay/pty-handler.ts` は先頭で `node-pty` を import している。ターミナルを動かすには疑似端末（PTY）の割り当てが要り、これは OS のシステムコールを叩く領域なので純粋な JavaScript では書けない。C++ のネイティブアドオンが必要になる。

この非対称が、症状の非対称に直結する。同ページは `make` / C++ コンパイラ / `python3` が無い場合の症状まで明示している。接続自体は完了し、**ファイル・git・エディタは動く。リモートターミナルだけが動かない**。

`fs` や `git` のハンドラは純粋な Node.js だけで動くので、Node さえあれば機能する。一方 `pty` ハンドラだけがネイティブモジュールに依存しているため、ビルドツールが無いとそこだけ欠ける。**責務が分かれているからこそ、部分的に死ぬ**わけだ。
...
### まとめると、Node.js が要る理由は 2 段構え

| 段階 | 欠けると | 症状 |
| --- | --- | --- |
| ① Node.js 本体（リレーを動かす） | 接続そのものが失敗 | `Node.js not found on remote host` |
| ② `build-essential` / `python3`（`node-pty` をビルドする） | 接続はできる | リモートターミナルだけ動かない |

次章で扱うのは **① の段階**だ。Node は入っているのに、非対話シェルでは PATH に載らないので Orca からは「無い」ように見える、という話になる。
...
- **リモートの Node.js は、エージェントのためではなく Orca 自身のために要る。** Claude Code も Codex CLI も今や Node を必要としないが、Orca がリモートに置くリレーが Node アプリケーションなので、無ければ接続の段階で落ちる。
- **公式ドキュメントにしか書かれていない前提**として、Linux リモートには `build-essential` と `python3` も要る。リレーのうち `node-pty` を使うターミナルだけがネイティブモジュールに依存するため、無いと「接続は成功するのにリモートターミナルだけ動かない」という切り分けにくい壊れ方をする。
