...
Agentic RAG は1回の質問で LLM を何度も呼び出します（Router 判定 → クエリ生成 → 関連性評価 → 統合…）。すべてを最上位モデルで回すとコストとレイテンシが跳ね上がるため、**役割ごとにモデルを使い分ける**のが定石です。バックエンドに求められる資質は次の3点です。
...
| モデル | 価格（入力 / 出力 per 1M） | Agentic RAG での役割 |
|---|---|---|
| **Claude Opus 4.8** | $5 / $25 | オーケストレーション全体、複雑な Planner・Synthesizer、長時間の自律ループ |
| **Claude Sonnet 5** | $3 / $15 | 常用の主力。コーディング・エージェント用途で Opus に迫る品質を Sonnet 価格で。Router・クエリ生成・統合 |
| **Claude Haiku 4.5** | $1 / $5 | 安く速い分類・評価。Router の質問分類、Self-Correction の関連性スコアリング |

### 3つのパターンへの割り当て例

- **Router の分類** → 軽い分類なので安価な小型モデル（Haiku 4.5 など）。低レイテンシが効く
- **クエリ生成 / Text-to-SQL** → 正確な SQL・検索クエリ生成には推論力が要るので中位モデル（Sonnet 5 など）
- **関連性評価（CRAG の grader）** → スコアリングは小型モデルで十分。独立した評価者を安く並列に回せる
- **Multi-step の Planner・Synthesizer** → 分解・矛盾整理・統合は最も知性を要するので中〜上位モデル（Sonnet 5、難しければ Opus 4.8）

### 実装上のポイント

- LangGraph / LlamaIndex はモデルに依存しないので、**ノードごとに別モデルを差し込む**構成にしやすい
- 判断タスク（Router 判定、関連性評価）では推論を有効にすると精度が上がる
- 単一モデルで固定するより、役割ごとの使い分けがコスト効率と品質のバランスに最も効く
