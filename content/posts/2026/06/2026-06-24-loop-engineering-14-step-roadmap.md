...
categories: ["AI/LLM"]
tags: ["ループエンジニアリング", "AIエージェント", "Claude Code", "Codex", "プロダクティビティ"]
---
...
```
従来: あなた → プロンプト → エージェント → 差分 → 確認 → 次のプロンプト
ループ: システム → タスク発見 → エージェント実行 → 検証 → 状態記録 → 次のアクション
```
...
![ループエンジニアリングのフリートループアーキテクチャ：オーケストレーターが複数のスペシャリストを並列に制御し、各スペシャリストがサブエージェントに作業を委譲する階層構造](/blogs/images/loop-engineering-fleet-loop.png)
