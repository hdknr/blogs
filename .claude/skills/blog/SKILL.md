...
7. PR を作成する（`--head` でブランチを明示指定し、`cd` を使わない）:
   まず PR 本文を worktree 内の一時ファイルに書き出し、`--body-file` で渡す（HEREDOC 内の `#` がセキュリティチェックに引っかかるのを回避）:
   ```bash
   mkdir -p "$WORKTREE_DIR/.claude/temp"
   cat > "$WORKTREE_DIR/.claude/temp/pr_body.md" <<'EOF'
   ## Summary
   - ...

   🤖 Generated with [Claude Code](https://claude.com/claude-code)
   EOF
   gh pr create --repo hdknr/blogs --head "$BRANCH_NAME" --title "Add blog: <記事タイトル>" --body-file "$WORKTREE_DIR/.claude/temp/pr_body.md"
   ```
...
9. **worktree は削除しない。** PR マージ後にユーザーが削除を指示するか、次回の `/blog` 実行時に削除する。マージ前の修正や並行作業に worktree を再利用できるようにするため。
...
**スクリプト `.claude/scripts/update-issue-comment.sh` を使用すること。** 個別コマンドで `.claude/temp/` にリダイレクト書き込みすると、センシティブファイル扱いで毎回同意が求められる。

```bash
bash .claude/scripts/update-issue-comment.sh {owner} {repo} {comment_id} <PR_URL>
```

例:
```bash
bash .claude/scripts/update-issue-comment.sh hdknr blogs 4126127772 https://github.com/hdknr/blogs/pull/141
```
...
2. worktree はそのまま残す（マージ前の修正や並行作業に再利用可能）
3. PR マージ後、ユーザーの指示で worktree を削除する:
   ```bash
