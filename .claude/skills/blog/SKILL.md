...
7. PR を作成する（`--head` でブランチを明示指定し、`cd` を使わない）:
   ```bash
   gh pr create --repo hdknr/blogs --head "$BRANCH_NAME" --title "Add blog: <記事タイトル>" --body "..."
   ```
...
1. 現在のコメント本文を一時ファイルに保存する:
   ```bash
   gh api /repos/{owner}/{repo}/issues/comments/{comment_id} --jq '.body' > /tmp/blog_comment_body.txt
   ```
2. PR リンクを末尾に追記する:
   ```bash
   printf '\n\n---\n📝 Blog: %s' "<PR_URL>" >> /tmp/blog_comment_body.txt
   ```
3. jq で JSON を構築し、`--input` で PATCH する:
   ```bash
   jq -n --rawfile body /tmp/blog_comment_body.txt '{body: $body}' | gh api /repos/{owner}/{repo}/issues/comments/{comment_id} --method PATCH --input -
   ```
4. 一時ファイルを削除する:
   ```bash
   rm /tmp/blog_comment_body.txt
   ```
