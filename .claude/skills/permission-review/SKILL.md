...
arguments:
  - name: log_file
    description: "同意ログファイルのパス（省略時は ~/Downloads/log.txt）"
    required: false
---
...
ログファイル（引数 `log_file` またはデフォルト `~/Downloads/log.txt`）を読み取る。
ログには以下のような形式で同意プロンプトが記録されている想定:
...
ログファイルから、同意を求められた各コマンドを抽出し、以下の情報を整理する:
