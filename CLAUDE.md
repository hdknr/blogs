...
Issue コメントのブログ化状態は **GitHub リアクション** で管理する:

- 🚀 (rocket) リアクション付き → **ブログ化済み**
- リアクションなし → **未ブログ化**
- `/blog` スキルで記事を作成した際、ソースコメントに自動で 🚀 を付与する
- 重複トピック等でブログ化を見送った場合も、手動で 🚀 を付けて管理する
- 未ブログ化コメント一覧: `gh api repos/hdknr/blogs/issues/{N}/comments --paginate --jq '.[] | select(.reactions.rocket == 0) | .html_url'`
